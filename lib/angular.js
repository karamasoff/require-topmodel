            ...
            var manager = {
                ...
                inject: function (httpResponse) {
                    var Model = httpResponse.config.model || false,
                        models = httpResponse.config.models || false,
                        polymodels = httpResponse.config.polymodels || false,
                        defered = $q.defer(),
                    ...
                    if (models) {
                        return $q.all(lodash.mapValues(models, function (model, key) {
                            var new_response = lodash.cloneDeep(httpResponse);
                            new_response.config.model = model;
                            new_response.config.models = null;
                            new_response.data = lodash.map(new_response.data, key);

                            return manager.inject(new_response);
                        }))
                            .then(function (results) {
                                var keys = lodash.keys(results);
                                var index = 0;
                                return lodash.map(lodash.zip.apply(lodash, lodash.values(results)), function (record) {
                                    return lodash.zipObject(keys, record);
                                }, []);
                            });
                    }
                    ...
                    return this.getInstance((Model || polymodels), response_data, httpResponse.config.polymorphKey);
                }
            };
        ...
        .provider('Api', function () {
            ...
            this.$get = ['$http', '$q', ModelService, function ($http, $q, MService) {
                var /**
                    ...
                    getRequestHandler = function (method, success, failure) {
                        return function (path, data, config) {
                            ...
                            // si on appelle une route en GET ou DELETE et qu'on a passé un hash de paramètres
                            ...
                            if (['get', 'delete'].indexOf(method) > -1 && data !== undefined) {
                ...
                return {
                    ...
                    delete: getRequestHandler('delete', passData, passResult),
                    setApiUrl: setApiUrl,
                    getApiUrl: function () {
                        return api_url;
                    }
